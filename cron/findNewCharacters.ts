...
export default {
    ...
    run: async () => {
        ...
        const highestCharacter = await Characters.findOne().sort({
            character_id: -1,
        });
        if (highestCharacter) {
        ...
        for (const characterId of newCharacterIds) {
            if (errorCount > 2) {
                return cliLogger.error(
                    "Too many errors while trying to find new characters, stopping"
                );
            }
        ...
        if (
            process.env.BACKEND_DISCORD_URL !== undefined &&
            newCharacters > 0
        ) {
