...
export default {
    ...
    description: "Clean up old access log statistics (hourly: 30 days, daily: 365 days)",
    ...
    run: async () => {
        ...
        try {
            ...
            const hourlyCutoff = new Date(now.getTime() - (30 * 24 * 60 * 60 * 1000)); // 30 days ago
            const dailyCutoff = new Date(now.getTime() - (365 * 24 * 60 * 60 * 1000)); // 365 days ago
            ...
            cliLogger.info(`Deleted ${hourlyResult.deletedCount} hourly access log stats older than 30 days`);
            ...
            cliLogger.info(`Deleted ${dailyResult.deletedCount} daily access log stats older than 365 days`);

            const totalDeleted = hourlyResult.deletedCount + dailyResult.deletedCount;
            cliLogger.info(`Access log stats cleanup completed. Total deleted: ${totalDeleted} records`);

            // Log remaining count for monitoring
            const remainingCount = await AccessLogStats.countDocuments();
            cliLogger.info(`Remaining access log stats in database: ${remainingCount}`);
