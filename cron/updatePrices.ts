    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            csvStream
                .on("data", (data) => {
                    ...
                    if (batch.length >= batchSize) {
                        ...
                        Prices.bulkWrite(batch, { ordered: false })
                            ...
                            .catch((error) => {
                                cliLogger.error(
                                    `Error inserting data for ${date}: ${error}`
                                );
                ...
                .on("end", () => {
                    if (batch.length > 0) {
                        Prices.bulkWrite(batch, { ordered: false })
                            ...
                            .catch((error) => {
                                cliLogger.error(
                                    `Error inserting data at the end for ${date}: ${error}`
                                );
                                resolve();
                            });
                    } else {
                ...
                .on("error", (error) => {
                    cliLogger.error(
                        `Error processing data for ${date}: ${error}`
                    );
                    reject(error);
                });
        });
...
function generateRecord(data: any) {
    return {
        ...
        order_count: data.order_count
            ? Number.parseInt(data.order_count, 10)
            : 0,
