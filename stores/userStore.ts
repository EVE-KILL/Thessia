  ...
  actions: {
    ...
    async login() {
      try {
        ...
        const { data } = await useFetch<{ authUrl: string }>('/api/auth/login', {
          query: {
    ...
    async logout() {
      try {
        ...
        this.authError = error instanceof Error ? error.message : 'Logout failed';
        return false;
      } finally {
        this.loading = false;
      }
    ...
    async verify() {
      try {
        ...
        this.authenticated = false;
        this.authError = error instanceof Error ? error.message : 'Verification failed';
        return false;
      } finally {
        this.loading = false;
      }
    ...
    async refreshToken() {
      try {
        ...
        this.authenticated = false;
        this.authError = error instanceof Error ? error.message : 'Token refresh failed';
        return false;
      } finally {
        this.loading = false;
      }
