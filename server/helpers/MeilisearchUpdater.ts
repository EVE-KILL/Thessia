...
export interface MeilisearchUpdaterOptions {
    progressReporter?: ((processed: number, total: number, entityType: EntityType) => string) | null
    getEstimatedCountsFn?: (entityType: EntityType) => Promise<number>
    batchSize?: number
}

export class MeilisearchUpdater {
    private readonly progressReporter: ((processed: number, total: number, entityType: EntityType) => string) | null
    private readonly getEstimatedCountsFn?: (entityType: EntityType) => Promise<number>
    private readonly client: Meilisearch
    private readonly batchSize: number

    constructor(options: MeilisearchUpdaterOptions = {}) {
        this.progressReporter = options.progressReporter === undefined ? null : options.progressReporter
        this.getEstimatedCountsFn = options.getEstimatedCountsFn
        this.client = new Meilisearch()
        this.batchSize = options.batchSize ?? DEFAULT_BATCH_SIZE
    }

    private async getEntities(entityType: EntityType, skip: number, limit: number): Promise<any[]> {
        switch (entityType) {
            case 'characters': {
            ...
            case 'regions': {
                const regions = await Regions.find(
                    ...
                    { region_id: 1, name: 1 }
                ).skip(skip).limit(limit).lean() as IRegion[]
                    ...
                    if (region.name && typeof region.name === 'object') {
                        ...
                        for (const lang of languages) {
                            translatedRegions.push({
                                ...
                                name: (region.name as any)[lang] || (region.name as any).en,
                                type: 'region',
                                rank: 2,
                                lang
                            })
            ...
            case 'items': {
                const items = await InvTypes.find(
                    ...
                    { type_id: 1, name: 1 }
                ).skip(skip).limit(limit).lean() as IInvType[]
                    ...
                    if (item.name && typeof item.name === 'object') {
                        ...
                        for (const lang of languages) {
                            translatedItems.push({
                                ...
                                name: (item.name as any)[lang] || (item.name as any).en,
                                type: 'item',
                                rank: 1,
                                lang
                            })
    ...
    private async processEntities(entityType: EntityType, estimatedTotal: number): Promise<number> {
        let count = 0
        let skip = 0
        let hasMore = true
        ...
        if (this.progressReporter && estimatedTotal > 0) {
            cliLogger.info(this.progressReporter(count, estimatedTotal, entityType))
        }

        while (hasMore) {
            const entities = await this.getEntities(entityType, skip, this.batchSize)
            if (entities.length > 0) {
                const res = await this.client.addDocuments('nitro-update', entities)
                ...
                if (taskStatus.status === 'failed') {
                    cliLogger.error(`Meilisearch task ${taskUid} failed for ${entityType}: ${taskStatus.error?.message ?? 'Unknown error'}`)
                ...
                if (this.progressReporter && estimatedTotal > 0) {
                    ...
                    if (currentPercentage !== lastLoggedPercentage || count === estimatedTotal) {
                        cliLogger.info(this.progressReporter(count, estimatedTotal, entityType))
                        lastLoggedPercentage = currentPercentage
                    }
                } else if (!this.progressReporter) {
                    cliLogger.info(`Processed ${count} ${entityType}... (total for this batch type so far)`)
                }
            }
            hasMore = entities.length === this.batchSize
        }

        if (this.progressReporter && estimatedTotal > 0 && count !== estimatedTotal) {
    ...
    public async runUpdate(): Promise<Record<EntityType, number>> {
        const entityTypes: EntityType[] = ['items', 'regions', 'systems', 'factions', 'alliances', 'corporations', 'characters']
        const mainIndexName = 'nitro'
        const updateIndexName = 'nitro-update'
        const processedCounts: Record<EntityType, number> = {} as Record<EntityType, number>

        cliLogger.info('Starting Meilisearch update process...')

        try {
            const indexExists = await this.client.existsIndex(updateIndexName)
            if (indexExists) {
                await this.client.deleteIndex(updateIndexName)
                cliLogger.info(`Deleted existing index: ${updateIndexName}`)
            }

            await this.client.createIndex(updateIndexName, { primaryKey: 'id' })
            cliLogger.info(`Created index: ${updateIndexName}`)

            const nitroIndexExists = await this.client.existsIndex(mainIndexName)
            if (!nitroIndexExists) {
                cliLogger.warn(`Main index ${mainIndexName} not found, creating...`)
                await this.client.createIndex(mainIndexName, { primaryKey: 'id' })
            }

            cliLogger.info(`Configuring settings for ${updateIndexName}...`)
            await this.client.updateRankingRules(updateIndexName, [
                'rank:asc',
                'words',
                'typo',
                'proximity',
            ...
            await this.client.updateFilterableAttributes(updateIndexName, [
                'type',
                'rank',
                'lang',
                'originalId'
            ])
            cliLogger.info(`Settings configured for ${updateIndexName}.`) // Changed from success to info

            let totalEstimatedDocs = 0
            const estimatedCountsPerEntity: Partial<Record<EntityType, number>> = {}

            if (this.getEstimatedCountsFn) {
                cliLogger.info('Calculating estimated document counts...')
                for (const entityType of entityTypes) {
                    try {
                        const count = await this.getEstimatedCountsFn(entityType)
                        estimatedCountsPerEntity[entityType] = count
                        totalEstimatedDocs += count
                        cliLogger.info(`  - ${entityType}: ${count.toLocaleString()}`)
                    } catch (e: any) {
                        cliLogger.error(`Error getting estimated count for ${entityType}: ${e.message}`)
                        estimatedCountsPerEntity[entityType] = 0 // Assume 0 if error
                    }
                }
                cliLogger.info(`Total estimated documents: ${totalEstimatedDocs.toLocaleString()}`)
            } else {
                cliLogger.info('No getEstimatedCountsFn provided, skipping detailed progress estimation.')
            }


            for (const entityType of entityTypes) {
                cliLogger.info(`Processing ${entityType}...`)
                const estimatedCountForType = estimatedCountsPerEntity[entityType] ?? 0 // Use 0 if no estimate function or error
                ...
                processedCounts[entityType] = await this.processEntities(entityType, estimatedCountForType)
            }

            cliLogger.info(`Swapping indexes: ${mainIndexName} <--> ${updateIndexName}`)
            await this.client.replaceIndex(mainIndexName, updateIndexName) // replaceIndex handles the swap and deletion of the old temp
            cliLogger.info(`Attempting to delete temporary index: ${updateIndexName} (if it wasn't automatically handled by replaceIndex)`)
            try {
                // replaceIndex should ideally handle this, but as a fallback:
                if (await this.client.existsIndex(updateIndexName)) {
                    await this.client.deleteIndex(updateIndexName)
                    cliLogger.info(`Temporary index ${updateIndexName} deleted successfully after swap.`) // Changed from success to info
                } else {
                    cliLogger.info(`Temporary index ${updateIndexName} was already deleted or handled by replaceIndex.`)
                }
            } catch (e: any) {
                cliLogger.warn(`Could not delete temporary index ${updateIndexName} after swap (it might have been already deleted): ${e.message}`)
            }


            cliLogger.info('Meilisearch update process completed successfully.') // Changed from success to info
            ...
            try {
                if (await this.client.existsIndex(updateIndexName)) {
                    cliLogger.info(`Attempting to delete ${updateIndexName} due to error...`)
                    await this.client.deleteIndex(updateIndexName)
                    cliLogger.info(`${updateIndexName} deleted after error.`) // Changed from success to info
                }
            } catch (cleanupError: any) {
                if (cleanupError.code !== 'index_not_found') { // Meilisearch specific error code
                    cliLogger.error(`Failed to delete ${updateIndexName} during error cleanup:`, cleanupError.message || cleanupError)
                }
            }
