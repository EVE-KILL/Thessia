...
async function processChunk(
    ...
    let queuedCount = 0;
    const affiliations: ICharacters[] = await fetchAffiliations(
        characters,
        attempt
    );
...
async function fetchAffiliations(characters: ICharacters[], attempts = 0) {
    ...
    try {
        ...
        if (attempts > 3) {
            cliLogger.error(
                `Failed to fetch affiliations for characters ${character_ids}`
            );
            ...
            const firstHalf = processChunk(
                characters.slice(0, half),
                firstHalfAttempts + 1
            );
            const secondHalf = processChunk(
