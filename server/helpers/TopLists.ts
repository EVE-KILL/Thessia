...
async function topCharacters(
  ...
  // Optimize query with early projection and $sum instead of $addToSet + $size
  const query: any[] = [
    { $match: matchFilter },
    // Early projection to reduce memory usage during processing
    { $project: { "attackers.character_id": 1, killmail_id: 1 } },
    { $unwind: "$attackers" },
    { $match: { "attackers.character_id": { $ne: 0 } } }, // Second filter after unwinding
    {
      $group: {
...
async function topCorporations(
  ...
  // Optimize query with early projection and $sum
  const query: any[] = [
    { $match: matchFilter },
    // Early projection to reduce memory usage during processing
    { $project: { "attackers.corporation_id": 1, killmail_id: 1 } },
    { $unwind: "$attackers" },
    { $match: { "attackers.corporation_id": { $ne: 0 } } }, // Second filter after unwinding
    {
      $group: {
...
async function topAlliances(
  ...
  const query: any[] = [
    ...
    { $project: { "attackers.alliance_id": 1, killmail_id: 1 } },
    { $unwind: "$attackers" },
    { $match: { "attackers.alliance_id": { $ne: 0 } } },
    {
      $group: {
...
async function topShips(
  ...
  const matchFilter: any = {
    kill_time: { $gte: calculatedTime },
    "attackers.ship_id": { $nin: [0, 670] }, // Exclude pods and default values
  };
  if (attackerType && typeId) {
    matchFilter[`attackers.${attackerType}`] = typeId;
  }
  ...
  const query: any[] = [
    { $match: matchFilter },
    // Early projection to reduce memory usage
    { $project: { "attackers.ship_id": 1 } },
    { $unwind: "$attackers" },
    { $match: { "attackers.ship_id": { $nin: [0, 670] } } }, // Second filter after unwinding
    {
      $group: {
...
async function topSolo(
  ...
  const query: any[] = [
    { $match: matchFilter },
    // Early projection to reduce memory usage
    { $project: { "attackers.character_id": 1, "attackers.final_blow": 1 } },
    { $unwind: "$attackers" },
