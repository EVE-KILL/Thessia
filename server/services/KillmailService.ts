...
export class KillmailService {
    ...
    /**
     * Find killmails by region with detailed fields for killlist
     */
    static async findByRegionDetailed(
        regionId: number,
        limit: number = 50,
        offset: number = 0
    ): Promise<any[]> {
    ...
    /**
     * Find killmails by systems and time range (for battle preview)
     */
    static async findBySystemsAndTimeRange(
        systemIds: number[],
        startTime: Date,
        endTime: Date
    ): Promise<any[]> {
    ...
    /**
     * Find recent killmails by victim ship type (for fitting analysis)
     */
    static async findRecentByVictimShipType(
        shipTypeId: number,
        days: number = 30,
        limit: number = 50
    ): Promise<any[]> {
    ...
    /**
     * Find latest killmails (for latest endpoint)
     */
    static async findLatest(limit: number = 10000) {
        return await prisma.killmail.findMany({
            select: {
                killmail_id: true,
                killmail_hash: true,
    ...
    /**
     * Find sibling killmails for a character in a system within a time window
     */
    static async findSiblings(
        victimCharacterId: number,
        excludeKillmailId: number,
        excludeShipTypeId: number,
        systemId: number,
        killTimeStart: Date,
        killTimeEnd: Date
    ) {
        return await prisma.killmail.findMany({
            where: {
                ...
                killmail_id: {
                    not: excludeKillmailId,
                },
                victim: {
                    character_id: victimCharacterId,
                    ship_type_id: {
                        not: excludeShipTypeId,
                    },
                },
            },
            select: {
