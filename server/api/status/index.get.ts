...
export default defineEventHandler(async () => {
    ...
    const [
        ...
        prisma.celestial.count(),
        prisma.character.count(),
        prisma.corporation.count(),
        prisma.comment.count(),
        prisma.constellation.count(),
        prisma.customPrice.count(),
        prisma.faction.count(),
        prisma.invFlag.count(),
        prisma.invGroup.count(),
        prisma.invType.count(),
        prisma.killmail.count(),
        prisma.killmail.count({ where: { processed: false } }),
        prisma.price.count(),
        prisma.region.count(),
        ...
        prisma.war.count(),
        prisma.race.count(),
        prisma.localScan.count(),
        prisma.campaign.count(),
        prisma.dScan.count(),
        prisma.stats.count(),
        prisma.savedQuery.count(),
        prisma.bloodline.count(),
        prisma.battle.count(),
        prisma.characterAchievements.count(),
        prisma.historicalStats.count(),
        prisma.config.count(),
    ]);

    // Cache stats are no-ops now that the runtime cache is removed
    const cacheSizes = {
        solarSystemsCache: 0,
        regionsCache: 0,
        constellationsCache: 0,
        customPriceCache: 0,
        invGroupsCache: 0,
        invFlagsCache: 0,
        invTypesCache: 0,
