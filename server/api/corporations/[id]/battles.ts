...
export default defineEventHandler(async (event) => {
    ...
    if (isNaN(corporationId)) {
        throw createError({ statusCode: 400, statusMessage: 'Invalid corporation ID' });
    }
    ...
    try {
        const matchStage: PipelineStage = { $match: { corporationsInvolved: corporationId } };
        ...
        throw createError({ statusCode: 500, statusMessage: 'Internal Server Error fetching corporation battles' });
    }
});
