...
export default defineEventHandler(async (event: H3Event) => {
    try {
        ...
        // Aggregate statistics with index hint
        const pipeline = [
            { $match: mongoQuery },
            {
                $group: {
                    _id: null,
                    totalKillmails: { $sum: 1 },
        ...
        const statsResult = await aggregationQuery;
        const stats = statsResult[0] || { totalKillmails: 0, iskDestroyed: 0 };
        ...
        console.error("Error fetching advanced view statistics:", error);
        ...
        throw createError({
            statusCode: 500,
            statusMessage: "Error fetching advanced view statistics",
            message: error.message || "Error fetching advanced view statistics",
        });
    }
});
