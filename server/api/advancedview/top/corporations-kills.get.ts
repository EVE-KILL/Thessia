...
export default defineCachedEventHandler(
    async (event) => {
        try {
            ...
            // Check if filters parameter exists and is valid
            ...
            const hint = { kill_time: -1 };

            // Aggregate top corporations by kills
            const pipeline: any[] = [
                { $match: mongoQuery },
                { $unwind: "$attackers" },
                {
            ...
            const topKillersByCorporation = await aggregationQuery;

            return { topKillersByCorporation };
        } catch (error: any) {
            console.error("Error fetching top corporations (kills):", error);
            ...
            throw createError({
                statusCode: 500,
                statusMessage: "Error fetching top corporations (kills)",
                message:
                    error.message || "Error fetching top corporations (kills)",
            });
        }
    },
    {
        ...
        getKey: (event) => {
            ...
            return `advancedview:top:corporations-kills:${filtersParam}`;
        },
    }
);
