...
// Shared helper functions (same as killers.get.ts)
...
export default defineCachedEventHandler(
    async (event) => {
        try {
            ...
            // Aggregate top victims by character
            const pipeline: any[] = [
                { $match: mongoQuery },
                {
                    $match: {
            ...
            const topVictimsByCharacter = await aggregationQuery;

            return { topVictimsByCharacter };
        } catch (error: any) {
            console.error("Error fetching top victims:", error);
            ...
            throw createError({
                statusCode: 500,
                statusMessage: "Error fetching top victims",
                message: error.message || "Error fetching top victims",
            });
        }
    },
    {
        ...
        getKey: (event) => {
            ...
            return `advancedview:top:victims:${filtersParam}`;
        },
    }
);
