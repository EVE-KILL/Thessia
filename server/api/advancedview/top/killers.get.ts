...
export default defineCachedEventHandler(
    async (event) => {
        try {
            ...
            // Aggregate top killers by character
            const pipeline: any[] = [
                { $match: mongoQuery },
                { $unwind: "$attackers" },
                {
            ...
            const topKillersByCharacter = await aggregationQuery;

            return { topKillersByCharacter };
        } catch (error: any) {
            console.error("Error fetching top killers:", error);
            ...
            throw createError({
                statusCode: 500,
                statusMessage: "Error fetching top killers",
                message: error.message || "Error fetching top killers",
            });
        }
    },
    {
        ...
        getKey: (event) => {
            ...
            return `advancedview:top:killers:${filtersParam}`;
        },
    }
);
