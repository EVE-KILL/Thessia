...
export default defineCachedEventHandler(async (event) => {
    ...
    if (!entityType || (entityType !== 'alliance' && entityType !== 'corporation')) {
    ...
    if ((listType === 'growing' || listType === 'shrinking') && !period) {
        event.node.res.statusCode = 400;
        return { error: 'Parameter "period" is required for listType "growing" or "shrinking".' };
    }

    const validPeriods = ['1d', '7d', '14d', '30d'];
    if (period && !validPeriods.includes(period)) {
        event.node.res.statusCode = 400;
        return { error: `Invalid period. Must be one of: ${validPeriods.join(', ')}.` };
    }
    ...
    if (isNaN(limit) || limit <= 0 || limit > 100) {
        event.node.res.statusCode = 400;
        return { error: 'Invalid limit. Must be a positive integer less than or equal to 100.' };
    }
    ...
    try {
        ...
        if (listType === 'newest') {
            ...
            if (entityType === 'alliance') {
                ...
                const statsMap = new Map(statsData.map(stat => [stat.alliance_id, stat]));
                ...
                return entityInfo.map(alliance => ({
                    ...
                    member_count: statsMap.get(alliance.alliance_id)?.count || 0,
                    change_1d: statsMap.get(alliance.alliance_id)?.change_1d || null,
                    change_7d: statsMap.get(alliance.alliance_id)?.change_7d || null,
                    change_14d: statsMap.get(alliance.alliance_id)?.change_14d || null,
                    change_30d: statsMap.get(alliance.alliance_id)?.change_30d || null,
                    pirate_members: statsMap.get(alliance.alliance_id)?.pirate_members || 0,
                    carebear_members: statsMap.get(alliance.alliance_id)?.carebear_members || 0,
                    sum_sec_status: statsMap.get(alliance.alliance_id)?.sum_sec_status || 0,
                ...
                const statsMap = new Map(statsData.map(stat => [stat.corporation_id, stat]));
                ...
                return entityInfo.map(corp => ({
                    ...
                    member_count: statsMap.get(corp.corporation_id)?.count || 0,
                    change_1d: statsMap.get(corp.corporation_id)?.change_1d || null,
                    change_7d: statsMap.get(corp.corporation_id)?.change_7d || null,
                    change_14d: statsMap.get(corp.corporation_id)?.change_14d || null,
                    change_30d: statsMap.get(corp.corporation_id)?.change_30d || null,
                    pirate_members: statsMap.get(corp.corporation_id)?.pirate_members || 0,
                    carebear_members: statsMap.get(corp.corporation_id)?.carebear_members || 0,
                    sum_sec_status: statsMap.get(corp.corporation_id)?.sum_sec_status || 0,
                    avg_sec_status: statsMap.get(corp.corporation_id)?.avg_sec_status || 0,
                }));
            }
        }
        ...
        const entityIds = historicalData.map(item =>
            entityType === 'alliance' ? item.alliance_id : item.corporation_id
        );
        ...
        if (entityType === 'alliance') {
            ...
            entityInfo = await Corporations.find({ corporation_id: { $in: entityIds } })
                .select('corporation_id name date_founded')
                .lean();
        }
        ...
        let finalResponse = historicalData.map(stat => {
            const entityId = entityType === 'alliance' ? stat.alliance_id : stat.corporation_id;
...
async function calculateWeightedSecurityRanking(
    ...
    const pipeline = [
        ...
        { $limit: limit },
        {
            $project: {
                _id: 0,
                entityId: rankingType === 'pirate'
                    ? (entityType === 'alliance' ? "$alliance_id" : "$corporation_id")
                    : (entityType === 'alliance' ? "$alliance_id" : "$corporation_id"),
                count: 1,
                pirate_members: 1,
                carebear_members: 1,
