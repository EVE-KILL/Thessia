...
export default defineCachedEventHandler(
    async (event: any) => {
        ...
        const attackerWhere =
            type === "character"
                ? { attackers: { some: { character_id: idNum } } }
                : type === "corporation"
                ? { attackers: { some: { corporation_id: idNum } } }
                : { attackers: { some: { alliance_id: idNum } } };
    ...
    },
    {
        ...
        getKey: (event: any) => {
            ...
            return `killlist:attacker:${type}:${id}:index:page:${page}:limit:${limit}`;
        },
    }
);
