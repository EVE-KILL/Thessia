...
export default defineEventHandler(async (event) => {
  try {
    ...
    cliLogger.debug(`Imgur proxy request for URL: ${url}, cache key: ${cacheKey}`);
    ...
    if (!response.ok) {
      cliLogger.error(`Failed to fetch Imgur JSON: ${response.status} ${response.statusText}`);
      return createError({
    ...
    cliLogger.debug(`Successfully resolved Imgur URL: ${cacheKey} → ${result.mediaUrl} (${result.mediaType})`);
 ...
 */
function extractMediaFromJson(imageData: any): { url: string, type: string } | null {
  try {
    ...
    if (imageData.is_album && imageData.album_images?.images?.length > 0) {
      ...
      } else if (ext === '.mp4' || ext === '.webm' || firstImage.has_sound) {
        type = 'video';
      }

      cliLogger.debug(`Resolved album media: ${hash}${ext} as ${type} (animated: ${firstImage.animated}, prefer_video: ${firstImage.prefer_video})`);
      return {
    ...
    // For single images
      ...
      cliLogger.debug(`Resolved single media: ${hash}${ext} as ${type} (animated: ${imageData.animated}, prefer_video: ${imageData.prefer_video})`);
      return {
        url: `https://i.imgur.com/${hash}${ext}`,
        type
      };
