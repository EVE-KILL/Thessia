import { getQuery } from "h3"; // Explicitly import getQuery and H3Event
...
interface IFittingGroup {
    ...
    count: number;
    ship_value: number;
    svg?: string; // Add optional svg property
}
...
export default defineCachedEventHandler(async (event) => {
    ...
    const finalFittings = sortedFittings.slice(0, limit).map((fit: IFittingGroup, index: number) => { // Use defined type
...
async function generateFittingCost(fitting: IFittingSlots): Promise<number> { // Use defined type
    let cost = 0;
    for (const slot in fitting) {
        for (const item of fitting[slot as keyof IFittingSlots]) { // Add type assertion
...
async function generateFitting(items: IItem[]): Promise<IFittingSlots> { // Use defined type
    const fitting: IFittingSlots = { // Use defined type
        high_slot: [],
        medium_slot: [],
        low_slot: [],
        rig_slot: [],
...
function pruneFittingForHash(fitting: IFittingSlots): any { // Use defined type
    const pruned: any = {};
    for (const slot in fitting) {
...
function generateSvg(
  ...
  <text x="70" y="30" font-family="Arial" font-size="12" fill="white">Fit Cost: ${formatNumber(
        Number(fitCost.toFixed(2)),
    )} ISK</text>
  <text x="70" y="45" font-family="Arial" font-size="12" fill="white">Ship Cost: ${formatNumber(
        Number(shipCost.toFixed(2)),
    )} ISK</text>
  <text x="70" y="60" font-family="Arial" font-size="12" fill="white">Total Cost: ${formatNumber(
