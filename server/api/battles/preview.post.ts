...
export default defineEventHandler(async (event) => {
    try {
        ...
        if (!systems || !Array.isArray(systems) || systems.length === 0) {
            throw createError({
                statusCode: 400,
                statusMessage: "apiErrors.customBattles.preview.systemRequired",
            });
        }
        ...
        if (systems.length > 5) {
            throw createError({
                statusCode: 400,
                statusMessage: "apiErrors.customBattles.preview.maxSystems",
            });
        }
        ...
        if (!startTime) {
            throw createError({
                statusCode: 400,
                statusMessage:
                    "apiErrors.customBattles.preview.startTimeRequired",
            });
        }
        if (!endTime) {
            throw createError({
                statusCode: 400,
                statusMessage:
                    "apiErrors.customBattles.preview.endTimeRequired",
            });
        }
        if (!sides || !Array.isArray(sides) || sides.length < 2) {
            throw createError({
                statusCode: 400,
                statusMessage: "apiErrors.customBattles.preview.minSides",
            });
        }
        if (sides.length > 4) {
            throw createError({
                statusCode: 400,
                statusMessage: "apiErrors.customBattles.preview.maxSides",
            });
        }
        ...
        throw createError({
            ...
            statusMessage: messageIsKey
                ? error.statusMessage
                : "apiErrors.customBattles.preview.internalServerError",
        });
    }
});
...
async function prepareTeamsData(sides: Side[]) {
    ...
    for (const side of sides) {
        const allianceIds = side.entities
            .filter((e) => e.type === "alliance")
            .map((e) => e.id);
        let corpIds = side.entities
            .filter((e) => e.type === "corporation")
            .map((e) => e.id);
        ...
        const filteredCorpIds = corpDocs
            .filter(
                (doc) =>
                    !doc.alliance_id || !allianceIds.includes(doc.alliance_id)
            )
