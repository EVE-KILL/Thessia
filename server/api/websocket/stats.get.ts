...
export default defineEventHandler(async (event) => {
  ...
  return {
    ...
    redis: {
      ...
      stats: {
        connected_clients: redisStats.clients?.connected_clients || 'unknown',
        used_memory: redisStats.memory?.used_memory_human || 'unknown',
        total_commands_processed: redisStats.stats?.total_commands_processed || 'unknown',
        keyspace_hits: redisStats.stats?.keyspace_hits || 'unknown',
        keyspace_misses: redisStats.stats?.keyspace_misses || 'unknown',
        total_connections_received: redisStats.stats?.total_connections_received || 'unknown',
        rejected_connections: redisStats.stats?.rejected_connections || 'unknown'
      }
    }
  };
});
