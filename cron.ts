...
async function main() {
    ...
    if (process.argv[2]) {
        ...
        if (job) {
            ...
            job.run({ args: [] })
                .catch((err: any) =>
                    cliLogger.error(`Error executing job "${jobName}": ${err}`)
                )
        ...
        for (const job of cronJobs) {
            try {
                ...
                if (Math.abs(now - prev.getTime()) <= tolerance) {
                    cliLogger.info(
                        `Running job "${job.name}" on schedule: ${job.schedule}`
                    );
                    runningJobs.push(
                        job.run({ args: [] }).catch((err: any) => {
                            cliLogger.error(
                                `Error in job "${job.name}": ${err}`
                            );
                            process.exit(1);
                        })
                    );
                ...
                cliLogger.error(
                    `Job "${job.name}" schedule error: ${job.schedule} - ${e}`
                );
            }
        }
