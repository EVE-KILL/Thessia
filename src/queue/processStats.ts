...
function validateJobData(job: Job): boolean {
    ...
    if (!entityType || !validTypes.includes(entityType)) {
        cliLogger.warn(
            `Invalid stats job: Missing or invalid entity type: ${entityType}`
        );
        return false;
    }
    ...
    if (
        ...
        cliLogger.warn(
            `Invalid stats job: Missing or invalid entity ID: ${entityId} for type ${entityType}`
        );
        return false;
    }
        ...
        createWorker(
            ...
            async (job: Job) => {
                ...
                if (!validateJobData(job)) {
                    cliLogger.info(
                        `Stats worker: Discarding invalid job #${job.id}`
                    );
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `Stats Processor: ${job?.id} ( EntityType: ${job?.data.entityType} | EntityID: ${job?.data.entityId} | Days: ${job?.data.days} ) | ${err.message}`
                );
            })
            .on("completed", (job: Job) => {
                cliLogger.info(
                    `Stats Processor: ${job.id} ( EntityType: ${job.data.entityType} | EntityID: ${job.data.entityId} | Days: ${job?.data.days} ) | Completed`
                );
            });
    },
};
