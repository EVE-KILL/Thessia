...
watchEffect(async () => {
    if (battleData.value) {
        ...
        // Convert dates to Unix timestamps (seconds since epoch) for the API
        const startTime = new Date(battle.value.start_time).getTime() / 1000;
        const endTime = new Date(battle.value.end_time).getTime() / 1000;
