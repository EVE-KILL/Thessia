<template>
  <Table
    ...
    :empty-text="t('killItems.noItems')"
  ...
  >
    ...
    <template #cell-name="{ item }">
      ...
      <div v-else-if="item.type === 'item' || item.type === 'container-item'"
           class="font-medium">{{ item.itemName }}</div>
      ...
      <div class="mobile-container">
        ...
        <div class="mobile-content">
          ...
          <div class="mobile-header">
            ...
            <div v-else-if="item.type === 'item' || item.type === 'container-item'"
                 class="font-medium mobile-title">{{ item.itemName }}</div>
            <div v-else-if="item.type === 'value'" class="font-medium mobile-title">{{ item.itemName }}</div>
          </div>
          ...
          <div v-if="item.type === 'item' || item.type === 'value'" class="mobile-details">
            <div class="mobile-badges-wrapper">
              <!-- Dropped badge -->
              <UBadge v-if="item.dropped > 0" variant="solid" color="success" class="item-badge item-badge-mobile">
                <span class="badge-text">{{ item.dropped }}</span>
              </UBadge>

              <!-- Destroyed badge -->
              <UBadge v-if="item.destroyed > 0" variant="solid" color="error" class="item-badge item-badge-mobile">
                <span class="badge-text">{{ item.destroyed }}</span>
              </UBadge>
            </div>
...
:deep(.container-item-row) {
  ...
  animation: simpleSlideDown 0.25s ease-in-out; /* Modified animation with no bounce */
...
:deep(.table-row) {
  margin-bottom: 0.25rem;
  transition: background-color 0.3s ease; /* Only transition background color, not position or size */
  position: relative;
...
:deep(.table-row) {
  ...
  padding-top: 0.125rem; /* Add padding instead */
...
:deep(.table-row)::before {
  content: "";
  position: absolute;
  inset: -1px; /* Slightly larger than the row itself */
