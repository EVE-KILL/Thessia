<template>
    <div class="kill-attackers">
        <!-- Key Attackers Tabs -->
        <UTabs v-model="activeKeyAttacker" :items="keyAttackerTabs" class="key-attackers-tabs" :ui="tabsUi"
            color="neutral">
            <!-- Final Blow Tab -->
                    ...
                    <div class="attacker-portrait">
                        ...
                        <div class="portraits-container">
                            ...
                            <NuxtLink v-if="finalBlowAttacker.corporation_id"
                                :to="`/corporation/${finalBlowAttacker.corporation_id}`">
                                <Image :type="'corporation'" :id="finalBlowAttacker.corporation_id" :size="32"
                                    class="portrait-image corporation-image" />
                            </NuxtLink>
                        </div>
                        ...
                        <div class="ship-portrait">
                            <NuxtLink v-if="finalBlowAttacker.ship_id" :to="`/item/${finalBlowAttacker.ship_id}`">
                                <Image :type="'type-render'" :id="finalBlowAttacker.ship_id" :size="64"
                                    class="portrait-image ship-image" />
                            </NuxtLink>
                        </div>
                    </div>
                    ...
                    <div class="attacker-details">
                        ...
                        <div class="entity-info">
                            <div class="entity-name primary">
                                <NuxtLink v-if="finalBlowAttacker.character_id"
                                    :to="`/character/${finalBlowAttacker.character_id}`" class="entity-link truncate">
                                    {{ finalBlowAttacker.character_name }}
                                </NuxtLink>
                                <span v-else class="entity-name truncate">{{ finalBlowAttacker.character_name ||
                                    $t('unknown') }}</span>
                            </div>
                        ...
                        <div class="weapon-info">
                            <div class="ship-info">
                                ...
                                <span v-else class="truncate">{{ getLocalizedString(finalBlowAttacker.ship_name,
            ...
            <template #top-damage="{ item }">
                <div class="attacker-portrait">
                    ...
                    <div class="portraits-container">
                        ...
                        <NuxtLink v-if="topDamageAttacker.corporation_id"
                            :to="`/corporation/${topDamageAttacker.corporation_id}`">
                ...
                <div class="attacker-details">
                    ...
                    <div class="entity-info">
                        <div class="entity-name primary">
                            <NuxtLink v-if="topDamageAttacker.character_id"
                                :to="`/character/${topDamageAttacker.character_id}`" class="entity-link truncate">
                                {{ topDamageAttacker.character_name }}
                            </NuxtLink>
                            <span v-else class="entity-name truncate">{{ topDamageAttacker.character_name ||
                                $t('unknown') }}</span>
                        </div>
                        <div class="entity-name secondary">
                            <NuxtLink v-if="topDamageAttacker.corporation_id"
                                :to="`/corporation/${topDamageAttacker.corporation_id}`" class="entity-link truncate">
                    ...
                    <div class="weapon-info">
                        <div class="ship-info">
                            ...
                            <span v-else class="truncate">{{ getLocalizedString(topDamageAttacker.ship_name,
                                currentLocale) || $t('unknown') }}</span>
                        </div>
                        <div class="weapon-detail">
                            <Icon name="lucide:zap" class="icon-small" />
        ...
        <div class="organizations-section">
            <div class="section-header" @click="toggleOrganizations">
                <h3 class="section-title">
                    {{ $t('organizationsInvolved') }}
                    <span class="org-counts">({{ organizationsTree.alliances.length }}A / {{ getTotalCorporationsCount()
                        }}C)</span>
                </h3>
            ...
            <div v-if="isOrganizationsOpen" class="organizations-content">
                ...
                <div v-for="(alliance, index) in organizationsTree.alliances" :key="`alliance-${index}`"
                    ...
                    <div class="corporation-list">
                        <div v-for="(corp, corpIndex) in alliance.corporations"
                ...
                <div v-if="organizationsTree.noAllianceCorporations.length > 0" class="no-alliance-section">
                    <div class="no-alliance-header">{{ $t('corporationsWithoutAlliance') }}</div>

                    <div class="corporation-list">
                        <div v-for="(corp, index) in organizationsTree.noAllianceCorporations"
        ...
        <div class="attackers-table">
            ...
            <div class="table-container">
                <div class="table-body compact-table">
                    <div v-for="(attacker, index) in sortedAttackers" :key="`attacker-${index}`" class="table-row"
                        ...
                        <div class="body-cell">
                            <NuxtLink v-if="attacker.character_id" :to="`/character/${attacker.character_id}`">
                                <Image :type="'character'" :id="attacker.character_id" :size="64"
                        ...
                        <div class="body-cell ship-images-cell -ml-1">
                            <div class="ship-images-container">
                                <NuxtLink v-if="attacker.ship_id" :to="`/item/${attacker.ship_id}`">
                                    <Image :type="'type-render'" :id="attacker.ship_id" :size="32"
                                        class="ship-render" />
                                </NuxtLink>
                                <NuxtLink v-if="attacker.weapon_type_id" :to="`/item/${attacker.weapon_type_id}`">
                                    <Image :type="'type-icon'" :id="attacker.weapon_type_id" :size="32"
                                        class="type-icon" />
                                </NuxtLink>
                            </div>
                        </div>
                        ...
                        <div class="body-cell info-cell">
                            <div class="attacker-info">
                                <div class="character-line">
                                    <NuxtLink v-if="attacker.character_id" :to="`/character/${attacker.character_id}`"
                                        class="character-name entity-link">
                                        {{ attacker.character_name || $t('unknown') }}
                                    </NuxtLink>
                                    <span v-else class="character-name">{{ attacker.character_name ||
                                        $t('unknown')
                                        }}</span>
                                ...
                                <div class="corporation-line">
                                    <NuxtLink v-if="attacker.corporation_id"
                                        :to="`/corporation/${attacker.corporation_id}`"
                                        class="corporation-name entity-link">
...
.attacker-details {
    flex: 1;
    min-width: 0;
    /* Ensure flexbox respects minimum width */
    display: flex;
    flex-direction: column;
...
.table-row.final-blow:hover,
.table-row.top-damage:hover {
