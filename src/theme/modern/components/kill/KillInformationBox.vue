        ...
        <div class="section ship-section">
            <div class="ship-image-container">
                <NuxtLink v-if="killmail?.victim?.ship_id" :to="`/item/${killmail.victim.ship_id}`">
                    <Image :type="'type-render'" :id="killmail.victim.ship_id" :size="96"
                        :alt="getLocalizedString(killmail?.victim?.ship_name)" class="ship-image" />
                </NuxtLink>
            </div>
            <div class="ship-info">
                <h3 class="ship-name">
                    ...
                    <div class="entity-info">
                        ...
                        <div v-if="killmail?.victim?.character_id" class="compact-entity-row">
                            <div class="entity-portraits">
                                <NuxtLink :to="`/character/${killmail.victim.character_id}`">
                                    <Image :type="'character'" :id="killmail.victim.character_id" :size="48"
                                        class="portrait-image" />
                                </NuxtLink>
                                <NuxtLink v-if="killmail.victim.corporation_id"
                                    :to="`/corporation/${killmail.victim.corporation_id}`">
                                    <Image :type="'corporation'" :id="killmail.victim.corporation_id" :size="32"
                        ...
                        <div v-if="killmail?.victim?.alliance_id" class="alliance-row">
                            <NuxtLink :to="`/alliance/${killmail.victim.alliance_id}`">
                                <Image :type="'alliance'" :id="killmail.victim.alliance_id" :size="24"
                                    class="small-icon" />
                            </NuxtLink>
                            <NuxtLink :to="`/alliance/${killmail.victim.alliance_id}`"
                                class="entity-link alliance-name truncate">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-content">
                            <div class="detail-primary">
                                <NuxtLink v-if="killmail?.system_id" :to="`/system/${killmail.system_id}`"
                                    class="entity-link truncate">
                            ...
                            <div class="detail-secondary">
                                <NuxtLink v-if="killmail?.region_id" :to="`/region/${killmail.region_id}`"
                                    class="entity-link truncate">
                                    {{ getLocalizedString(killmail?.region_name) }}
                                </NuxtLink>
                                <span v-else class="truncate">{{ getLocalizedString(killmail?.region_name) }}</span>
                            </div>
                        </div>
                    </div>
...
.entity-column {
    width: 100%;
    /* Ensure column takes full width */
}
...
.entity-details {
    ...
    width: calc(100% - 60px);
    /* Account for portrait width and gap */
}
