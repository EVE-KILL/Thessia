...
export function useEveImages() {
  ...
  const getCharacterPortrait = (characterId: number | null, size = 128): string | null => {
    ...
    return `https://images.evetech.net/characters/${characterId}/portrait?size=${validSize}`;
  };
   ...
   */
  const getCorporationLogo = (corporationId: number | null, size = 64): string | null => {
    if (!corporationId) return null;
    const validSize = normalizeSize(size);
    return `https://images.evetech.net/corporations/${corporationId}/logo?size=${validSize}`;
  };
   ...
   */
  const getAllianceLogo = (allianceId: number | null, size = 64): string | null => {
    if (!allianceId) return null;
    const validSize = normalizeSize(size);
    return `https://images.evetech.net/alliances/${allianceId}/logo?size=${validSize}`;
  };
  ...
  const getTypeIcon = (typeId: number | null, size = 64): string | null => {
    ...
    const validSize = normalizeSize(size);
    return `https://images.evetech.net/types/${typeId}/icon?size=${validSize}`;
  };
   ...
   */
  const getTypeRender = (typeId: number | null, size = 512): string | null => {
    if (!typeId) return null;
    const validSize = normalizeSize(size);
