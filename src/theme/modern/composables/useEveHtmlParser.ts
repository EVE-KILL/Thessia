...
export const useEveHtmlParser = () => {
  ...
  const STATION_TYPE_IDS = [
    54, 56, 57, 58, 59, 1529, 1530, 1531, 1926, 1927, 1928, 1929, 1930, 1931, 1932,
    2071, 2496, 2497, 2498, 2499, 2500, 2501, 2502, 3864, 3865, 3866, 3867, 3868,
    3869, 3870, 3871, 3872, 4023, 4024, 9856, 9857, 9867, 9868, 9873, 10795, 12242,
    12294, 12295, 19757, 21642, 21644, 21645, 21646, 22296, 22297, 22298, 29323,
    29387, 29388, 29389, 29390, 34325, 34326, 52678, 59956, 71361, 74397
  ];
  ...
  const convertEveHtml = (htmlContent: string): string => {
    ...
    content = content
      ...
      .replace(/<font\s+color=["']?([^"'\s>]+)["']?>(.*?)<\/font>/gi, '<span style="color: $1">$2</span>')
      ...
      // Size tags
      .replace(/<size=["']?([^"'\s>]+)["']?>(.*?)<\/size>/gi, '<span style="font-size: $1">$2</span>')
      ...
      content = content.replace(/<a href="killReport:(\d+)(:[A-Fa-f0-9]+)?"[^>]*>([^<]+)<\/a>/g,
        '<a href="/kill/$1">$3</a>');
      content = content.replace(/<a href="warReport:(\d+)"[^>]*>([^<]+)<\/a>/g,
        '<a href="/war/$1">$2</a>');

      // Process showinfo links
      content = content.replace(/<a href="(showinfo:[^"]+)"[^>]*>([^<]+)<\/a>/g,
