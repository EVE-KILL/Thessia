...
export default {
    ...
    description:
        "Updates campaign statistics - hourly for active campaigns, daily for completed ones",
    schedule: "0 * * * *", // Run every hour
    run: async () => {
        const now = new Date();
        cliLogger.info(
            `🔄 Starting campaign update process at ${now.toISOString()}`
        );
        ...
        try {
            ...
            cliLogger.info(
                `📊 Found ${campaignsToUpdate.length} campaigns that need updating`
            );
            ...
            for (const campaign of campaignsToUpdate) {
                try {
                    await queueCampaignProcessing(
                        campaign.campaign_id,
                        getPriority(campaign, now)
                    );
                    queuedCount++;
                    cliLogger.info(
                        `✅ Queued campaign ${campaign.campaign_id} (${campaign.name}) for processing`
                    );
                } catch (error: any) {
                    cliLogger.error(
                        `❌ Failed to queue campaign ${campaign.campaign_id}: ${
            ...
            cliLogger.info(
                `🚀 Successfully queued ${queuedCount}/${campaignsToUpdate.length} campaigns for processing`
            );
        } catch (error: any) {
            cliLogger.error(
                `💥 Campaign updater failed: ${
                    error?.message || "Unknown error"
                }`
            );
        }
...
function shouldUpdateCampaign(campaign: any, now: Date): boolean {
    ...
    const lastProcessed = campaign.last_processed_at
        ? new Date(campaign.last_processed_at)
        : null;
    const processingCompleted = campaign.processing_completed_at
    ...
    if (
        ...
        campaign.processing_status === "failed" ||
...
function getCampaignPhase(
    ...
    endTime: Date | null,
    now: Date
): "future" | "active" | "recently_ended" | "old" {
