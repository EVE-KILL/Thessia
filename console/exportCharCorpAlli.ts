...
export default {
    ...
    run: async () => {
        ...
        await Characters.find({})
            ...
            .eachAsync((doc) => {
                processedChars++;
                if (processedChars % 100 === 0)
                    console.log(`Characters: ${processedChars}/${charCount}`);
                if (!firstChar)
                    fs.appendFileSync(
                        path.join(__dirname, "../server/data/characters.json"),
                        ",\n"
        ...
        await Corporations.find({})
            ...
            .eachAsync((doc) => {
                processedCorps++;
                if (processedCorps % 100 === 0)
                    console.log(`Corporations: ${processedCorps}/${corpCount}`);
                if (!firstCorp)
                    fs.appendFileSync(
                        path.join(
                            __dirname,
                            "../server/data/corporations.json"
                        ),
                        ",\n"
                    );
                fs.appendFileSync(
        ...
        await Alliances.find({})
            ...
            .eachAsync((doc) => {
                processedAlli++;
                if (processedAlli % 100 === 0)
                    console.log(`Alliances: ${processedAlli}/${alliCount}`);
                if (!firstAlli)
                    fs.appendFileSync(
                        path.join(__dirname, "../server/data/alliances.json"),
                        ",\n"
