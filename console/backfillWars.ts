...
export default {
    ...
    run: async () => {
        ...
        while (hasMoreWars) {
            ...
            console.log(
                `ℹ️  Fetched ${wars.length} wars with latestWarId: ${latestWarId}`
            );
            ...
            for (const warId of wars) {
                ...
                const existingWar: IWar | null = await Wars.findOne({
                    war_id: warId,
                });
                if (existingWar) {
                    continue;
        ...
        console.log(
            `✅ Finished fetching wars. Total war IDs fetched: ${allWarIds.length}`
        );
...
async function getWars(latestWarId: number) {
    const wars = await esiFetcher(
        ...
        }latest/wars/?max_war_id=${latestWarId}&datasource=tranquility`
    );
    return wars;
}
