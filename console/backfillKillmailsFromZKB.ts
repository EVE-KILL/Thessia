    ...
    run: async () => {
        ...
        for (const [date] of Object.entries(data).reverse()) {
            ...
            const existingIds = new Set(
                existingKillmails.map((k) => k.killmail_id)
            );
            ...
            console.log(
                `Found ${missingKillmails.length} missing killmails for ${date}`
            );
            killmailQueue.addBulk(
                missingKillmails.map(([killmail_id, killmail_hash]) => ({
                    name: "processKillmail",
                    data: {
                        killmailId: Number(killmail_id),
                        killmailHash: killmail_hash as string,
                    },
                    opts: { priority: 100 },
                }))
            );
            ...
            console.log(
                `Queued ${missingKillmails.length} killmails for ${date}`
            );
