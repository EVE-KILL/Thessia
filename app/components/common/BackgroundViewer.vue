<template>
    <div class="background-view-button">
        <UButton :icon="isViewingBackground ? 'lucide:eye-off' : 'lucide:eye'" color="neutral" variant="solid" size="sm"
            aria-label="View background" :title="$t('toggleVisibility')" @click="toggleBackgroundView" />
    </div>
</template>
...
const toggleBackgroundView = () => {
    ...
    if (isViewingBackground.value) {
      ...
      /* Keep the buttons visible with a nice transition */
      ...
      /* Disable pointer events except for our buttons */
      body {
