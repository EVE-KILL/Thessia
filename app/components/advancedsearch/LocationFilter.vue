<template>
    <div>
        ...
        <div class="mb-4">
            <ButtonGroup v-model="locationTypeArray" :options="locationTypeOptions" class="flex gap-2"
                @update:model-value="onLocationTypeChange" />
        </div>

        <!-- Location Search -->
        <div v-if="location.type" class="relative">
            <input v-model="searchTerm" type="text" :placeholder="`Search for ${location.type}...`"
            ...
            <div v-if="searchResults.length > 0"
                ...
                <div v-for="result in searchResults" :key="result.id"
                    class="px-3 py-2 hover:bg-zinc-700 cursor-pointer text-zinc-100" @click="selectLocation(result)">
                    {{ result.name }}
                </div>
            </div>
        </div>
        ...
        <div v-if="location.name" class="mt-3 p-3 bg-zinc-800 border border-zinc-600 rounded-lg">
            <div class="flex items-center justify-between">
                ...
                <button @click="clearLocation" class="text-zinc-400 hover:text-zinc-200">
            ...
            <div v-if="location.type" class="mt-3">
                ...
                <ButtonGroup v-model="location.securityTypes" :options="securityOptions" multiple
