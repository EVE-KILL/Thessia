<template>
    <div>
        ...
        <div v-if="pending || isLoading || (stats && stats.processing)"
        ...
        <div v-else-if="stats?.processing && stats?.status === 'failed'"
            class="flex flex-col items-center justify-center py-12">
            ...
            <div class="hidden lg:flex lg:flex-row gap-6">
                ...
                <div class="lg:w-4/5">
                    ...
                    <div v-if="stats.mostValuableKills && stats.mostValuableKills.length > 0" class="mb-6">
                        <CampaignMostValuable :title="t('campaign.most_valuable_kills')"
                            :items="stats.mostValuableKills" :limit="7" :loading="pending" />
                    </div>
                ...
                <div class="lg:w-1/5">
                    ...
                    <div v-if="hasAttackerDefinitions && stats.topKillersByCharacter?.length"
                        class="mb-6 campaign-sidebar-box">
                    ...
                    <div v-if="hasVictimDefinitions && stats.topVictimsByCharacter?.length"
                        class="mb-6 campaign-sidebar-box">
                        <CampaignTopBox :title="t('campaign.top_victims')" :entities="stats.topVictimsByCharacter"
                            :countField="'losses'" :countTitle="t('losses')" entityType="character" :loading="pending"
                            :limit="10" />
                    </div>

                    <!-- Top Damage Dealers Box - Show if we have attackers  -->
                    <div v-if="hasAttackerDefinitions && stats.topDamageDealersByCharacter?.length"
                        class="mb-6 campaign-sidebar-box">
                    ...
                    <div v-if="hasVictimDefinitions && stats.topDamageTakersByCharacter?.length"
                        class="mb-6 campaign-sidebar-box">
                    ...
                    <div v-if="stats.topKillersByAlliance?.length" class="mb-6 campaign-sidebar-box">
                        <CampaignTopBox :title="t('campaign.top_killer_alliances')"
                            :entities="stats.topKillersByAlliance" :countField="'kills'" :countTitle="t('kills')"
                            entityType="alliance" :loading="pending" :limit="10" />
                    </div>
                    ...
                    <div v-if="stats.topVictimsByAlliance?.length" class="mb-6 campaign-sidebar-box">
                        <CampaignTopBox :title="t('campaign.top_victim_alliances')"
                            :entities="stats.topVictimsByAlliance" :countField="'losses'" :countTitle="t('losses')"
                            entityType="alliance" :loading="pending" :limit="10" />
                    </div>
                    ...
                    <div v-if="stats.topKillersByCorporation?.length" class="mb-6 campaign-sidebar-box">
                        <CampaignTopBox :title="t('campaign.top_killer_corporations')"
                            :entities="stats.topKillersByCorporation" :countField="'kills'" :countTitle="t('kills')"
                            entityType="corporation" :loading="pending" :limit="10" />
                    </div>
                    ...
                    <div v-if="stats.topVictimsByCorporation?.length" class="mb-6 campaign-sidebar-box">
                        <CampaignTopBox :title="t('campaign.top_victim_corporations')"
                            :entities="stats.topVictimsByCorporation" :countField="'losses'" :countTitle="t('losses')"
                            entityType="corporation" :loading="pending" :limit="10" />
                    </div>
                </div>
            </div>
                ...
                <Tabs v-model="activeTabId" :items="tabItems" :ui="tabsUi" color="neutral"
                    ...
                    <template #ships>
                        <div class="mt-4 space-y-6">
                            ...
                            <div v-if="stats.mostValuableKills && stats.mostValuableKills.length > 0">
                                <CampaignMostValuable :title="t('campaign.most_valuable_kills')"
                                    :items="stats.mostValuableKills" :limit="4" :loading="pending" />
                            </div>
                        </div>
                    </template>
                            ...
                            <div v-if="stats.topKillersByAlliance?.length || stats.topVictimsByAlliance?.length">
                                <div class="space-y-4">
                                    <CampaignTopBox v-if="stats.topKillersByAlliance?.length"
                                        ...
                                        :entities="stats.topKillersByAlliance" :countField="'kills'"
                                        :countTitle="t('kills')" entityType="alliance" :loading="pending" :limit="10" />

                                    <CampaignTopBox v-if="stats.topVictimsByAlliance?.length"
                                        :title="t('campaign.top_victim_alliances')"
                                        :entities="stats.topVictimsByAlliance" :countField="'losses'"
                                        :countTitle="t('losses')" entityType="alliance" :loading="pending"
                                        :limit="10" />
                                </div>
                            </div>
                            ...
                            <div v-if="stats.topKillersByCorporation?.length || stats.topVictimsByCorporation?.length">
                                <div class="space-y-4">
                                    <CampaignTopBox v-if="stats.topKillersByCorporation?.length"
                                        ...
                                        :entities="stats.topKillersByCorporation" :countField="'kills'"
                                        :countTitle="t('kills')" entityType="corporation" :loading="pending"
                                        :limit="10" />

                                    <CampaignTopBox v-if="stats.topVictimsByCorporation?.length"
                                        :title="t('campaign.top_victim_corporations')"
                                        :entities="stats.topVictimsByCorporation" :countField="'losses'"
                                        :countTitle="t('losses')" entityType="corporation" :loading="pending"
                                        :limit="10" />
                                </div>
                            </div>
                    ...
                    <template #characters>
                        <div class="mt-4 space-y-6">
                            ...
                            <div v-if="hasVictimDefinitions && stats.topVictimsByCharacter?.length"
                                class="mb-6 campaign-sidebar-box">
                                <CampaignTopBox :title="t('campaign.top_victims')"
                                    :entities="stats.topVictimsByCharacter" :countField="'losses'"
                                    :countTitle="t('losses')" entityType="character" :loading="pending" :limit="10" />
                            </div>
                    ...
                    <template #kills>
                        <div class="w-full">
                            <CampaignKillList :campaignId="campaignId" :campaignQuery="stats?.campaignQuery"
                                :limit="25" />
                        </div>
                    </template>
                </Tabs>
