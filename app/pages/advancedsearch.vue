    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="bg-zinc-900 border border-zinc-700 rounded-lg p-6 mb-6">
        <div class="mb-4 relative">
          <input v-model="searchTerm" @input="onSearchInput" @focus="showSearchResults = true" @blur="hideSearchResults"
            type="text" placeholder="Search for pilots, corporations, alliances, or ships..."
                ...
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex-shrink-0">
                      <Image v-if="result.type === 'character'" type="character" :id="result.id" :alt="result.name"
                        size="32" class="w-8 h-8" :rounded="true" />
                      <Image v-else-if="result.type === 'corporation'" type="corporation" :id="result.id"
                        :alt="result.name" size="32" class="w-8 h-8" :rounded="true" />
                      <Image v-else-if="result.type === 'alliance'" type="alliance" :id="result.id" :alt="result.name"
                        size="32" class="w-8 h-8" :rounded="true" />
                      <Image v-else-if="result.type === 'ship'" type="type-icon" :id="result.id" :alt="result.name"
                        size="32" class="w-8 h-8" :rounded="true" />
                      <Image v-else-if="result.type === 'item'" type="item" :id="result.id" :name="result.name"
                        :alt="result.name" size="32" class="w-8 h-8" :rounded="true" />
                      <div v-else class="w-8 h-8 bg-zinc-600 rounded flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <div class="text-zinc-400 text-sm flex items-center gap-2">
                        ...
                        <span v-if="(result.type === 'corporation' || result.type === 'alliance') && result.ticker"
      ...
      <div class="bg-zinc-900 border border-zinc-700 rounded-lg p-6 mb-6">
        ...
        <div class="flex flex-wrap gap-2">
          <button v-for="quickFilter in quickFilters" :key="quickFilter.key" @click="applyQuickFilter(quickFilter.key)"
          ...
          <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
              ...
              <div v-if="filters.location.type && filters.location.value" class="mb-2">
                <div class="inline-flex items-center bg-green-600 text-white text-sm px-3 py-1 rounded-full">
                  <span>{{ filters.location.type.charAt(0).toUpperCase() + filters.location.type.slice(1) }}: {{
                    filters.location.value }}</span>
                  <button @click="clearLocationFilter" class="ml-2 hover:bg-green-700 rounded-full p-1">
            ...
            <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
              ...
              <div class="flex flex-wrap gap-1">
                <button v-for="option in attackerCountOptions" :key="option.value"
                  ...
                  :class="getFilterButtonClass(filters.attackerCount === option.value)" class="text-xs px-2 py-1">
                  {{ option.label }}
                </button>
              </div>
            </div>
            ...
            <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
              ...
              <div class="flex flex-wrap gap-1">
                <button v-for="option in attackerTypeOptions" :key="option.value"
                  ...
                  :class="getFilterButtonClass(filters.attackerType === option.value)" class="text-xs px-2 py-1">
                  {{ option.label }}
                </button>
              </div>
            </div>
            ...
            <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
              ...
              <div class="flex flex-wrap gap-1">
                <button v-for="option in iskValueOptions" :key="option.value"
                  ...
                  :class="getFilterButtonClass(filters.iskValue === option.value)" class="text-xs px-2 py-1">
                  {{ option.label }}
                </button>
              </div>
            </div>
            ...
            <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
              ...
              <div class="flex flex-wrap gap-1">
                <button v-for="option in shipCategoryOptions" :key="option.value"
                  ...
                  :class="getFilterButtonClass(filters.shipCategory === option.value)" class="text-xs px-2 py-1">
                  {{ option.label }}
                </button>
              </div>
            </div>
            ...
            <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
              ...
              <div class="flex flex-wrap gap-1 mb-3">
                <button v-for="option in timeFilterOptions" :key="option.value"
                  ...
                  :class="getFilterButtonClass(filters.timeFilter === option.value)" class="text-xs px-2 py-1">
                  {{ option.label }}
                </button>
              </div>
      ...
      <div class="bg-zinc-900 border border-zinc-700 rounded-lg p-6 mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              <button v-for="option in sortFieldOptions" :key="option.value" @click="sortField = option.value"
          ...
          <div>
            ...
            <div class="flex gap-2">
              <button @click="sortDirection = 'desc'" :class="getFilterButtonClass(sortDirection === 'desc')"
                class="text-sm px-3 py-2">
                Descending
              </button>
              <button @click="sortDirection = 'asc'" :class="getFilterButtonClass(sortDirection === 'asc')"
                class="text-sm px-3 py-2">
                Ascending
              </button>
            </div>
        ...
        <div class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
          ...
          <div class="flex flex-wrap gap-2">
            <div v-for="(item, index) in filters.items" :key="item.id"
              ...
              <span>{{ item.name }}</span>
              <button @click="removeItem(index)" class="text-purple-200 hover:text-white transition-colors">
        ...
        <div class="bg-zinc-900 border border-zinc-700 rounded-lg p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="text-sm text-zinc-400">
              {{ queryResult?.length || 0 }} {{ (queryResult?.length || 0) === 1 ? 'killmail' : 'killmails' }} found
            </div>
          </div>
          ...
          <!-- Results Container -->
          <div v-if="queryResult && queryResult.length > 0" class="bg-zinc-800 border border-zinc-600 rounded-lg p-4">
            <KillList :externalKilllistData="transformedQueryResult" :limit="25" :enablePagination="false" wsDisabled />
          </div>
