    ...
    run: () => {
        ...
        createWorker(
            ...
            async (job: Job) => {
                ...
                if (
                    warData.aggressor.ships_killed > 0 ||
                    warData.defender.ships_killed > 0
                ) {
                    cliLogger.info(
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `War Update: ${job?.id} ( WarID: ${job?.data.warId} ) | ${
                        err.message
                    } | ${
                        process.env.ESI_URL || "https://esi.evetech.net/"
                    }latest/wars/${job?.data.warId}/`
                );
            })
            .on("completed", (job: Job) => {
                cliLogger.info(
                    `War Update: ${job.id} ( WarID: ${job.data.warId} ) | Completed`
                );
            });
    },
};
