...
} from "../server/queue/Character";
import {
    updateCorporation,
    updateCorporationHistory,
} from "../server/queue/Corporation";
    ...
    run: () => {
        ...
        createWorker(
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `Character Update: ${job?.id} ( CharacterID: ${
                        job?.data.characterId
                    } ) | ${err.message} | ${
                        process.env.ESI_URL || "https://esi.evetech.net/"
                    }latest/characters/${job?.data.characterId}/`
                );
            })
        ...
        createWorker(
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `Corporation Update: ${job?.id} ( CorporationID: ${
                        job?.data.corporationId
                    } ) | ${err.message} | ${
                        process.env.ESI_URL || "https://esi.evetech.net/"
                    }latest/corporations/${job?.data.corporationId}/`
                );
            })
            .on("completed", (job: Job) => {
                cliLogger.info(
        ...
        createWorker(
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `Alliance Update: ${job?.id} ( AllianceID: ${
                        job?.data.allianceId
                    } ) | ${err.message} | ${
                        process.env.ESI_URL || "https://esi.evetech.net/"
                    }latest/alliances/${job?.data.allianceId}/`
                );
            })
            .on("completed", (job: Job) => {
                cliLogger.info(
        ...
        createWorker(
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `Character History Update: ${job?.id} ( CharacterID: ${
                        job?.data.characterId
                    } ) | ${err.message} | ${
                        process.env.ESI_URL || "https://esi.evetech.net/"
                    }latest/characters/${job?.data.characterId}/`
        ...
        createWorker(
            ...
            .on("failed", (job: Job | undefined, err: Error) => {
                cliLogger.error(
                    `Corporation History Update: ${job?.id} ( CorporationID: ${
                        job?.data.corporationId
                    } ) | ${err.message} | ${
                        process.env.ESI_URL || "https://esi.evetech.net/"
                    }latest/corporations/${job?.data.corporationId}/`
